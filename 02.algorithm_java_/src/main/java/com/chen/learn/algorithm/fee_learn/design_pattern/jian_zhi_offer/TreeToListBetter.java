...
public class TreeToListBetter {
    ...
    public TreeNode Convert(TreeNode pRootOfTree) {
        ...
        midIterator(pRootOfTree, new TreeNode[1]);
     ...
     * @param last
     */
    public void midIterator(TreeNode root, TreeNode[] last) {
        if (null == root) {
            return;
        }
        midIterator(root.left, last);
        Optional.ofNullable(last[0]).ifPresent(e -> {
            e.right = root;
            root.left = e;
        });
        last[0]=root;
        midIterator(root.right, last);
    }
    ...
    public static void main(String[] args) {
        ...
        printA(new TreeToListBetter().Convert(root));
