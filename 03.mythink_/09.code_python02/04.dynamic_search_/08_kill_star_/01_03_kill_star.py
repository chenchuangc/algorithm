...
group_list = [Group(1, 1), Group(2, 4), Group(3, 3), Group(1, 1), Group(3, 4)]


def get_best_score():
    global group_list
    dic = [[[0] * 9 for i in range(5)] for j in range(5)]
    for i in range(len(group_list)):
        g = group_list[i]
        dic[i][i][g.num] = g.num * g.num
    dic[0][0][2] = 4
    dic[2][2][7] = 49
    # print dic
    ...
    for n in range(2, num + 1):
        for i in range(num - n + 1):
            ...
            cur2 = 0
            list_another = []
            for index in range(len(dic[0][0])):
                if dic[j][j][index] != 0:
                    list_another.append(index)
            # list_another.append()
            print list_another
            for ele in list_another:
                cur1 = dic[i][j - 1][pre_g.num] + dic[j][j][ele]
                for k in range(i, j):
                    if group_list[k].color == group_list[j].color:
                        k_g = group_list[k]
